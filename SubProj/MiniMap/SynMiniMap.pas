...
uses
  ...
  , Controls
  , Graphics
  , SynEdit, SynEditPlugins, StdCtrls;
...
type
  ...
  public
    procedure StretchDrawHalftone(const AX, AY, AWidth, AHeight: integer; const ASource: TCanvas;
      const ASrcX, ASrcY, ASrcWidth, ASrcHeight: integer); overload;
    procedure StretchDrawHalftone(const APoint: TPoint; const AWidth, AHeight: integer; const ASource: TCanvas;
...
type
  ...
  protected
    ...
    procedure AfterPaint(ACanvas: TCanvas; const AClip: TRect; FirstLine: integer; LastLine: integer); virtual;
 {   procedure LinesDeleted(FirstLine: Integer; Count: Integer); virtual; dynamic;
    procedure LinesInserted(FirstLine: Integer; Count: Integer);}
  private
...
procedure TSynMiniMapCanvasHelper.StretchDrawHalftone(const AX, AY, AWidth, AHeight: integer; const ASource: TCanvas;
...
procedure TSynMiniMapCanvasHelper.StretchDrawHalftone(const APoint: TPoint; const AWidth, AHeight: integer;
  const ASource: TCanvas; const ASrcPoint: TPoint; ASrcWidth, ASrcHeight: integer);
begin
  StretchDrawHalftone(
    APoint.X, APoint.Y,
    AWidth, AHeight,
...
procedure TSynMiniMapEditorPlugin.AfterPaint(ACanvas: TCanvas; const AClip: TRect; FirstLine, LastLine: integer);
begin
  inherited;
  FSynMiniMap.Render;
end;
...
begin
  inherited BeforeEditorChange;
  //FSynMiniMap.Render;
end;

procedure TSynMiniMapEditorPlugin.AfterEditorChange;
begin
  inherited AfterEditorChange;
  //FSynMiniMap.Render;
end;
...
procedure TSynMiniMap.CNVScroll(var Message: TLMVScroll);
begin
    ResetInternals;
end;
...
begin
  ...
  if NOT Options.AllowScroll then
    Exit;
  if Scrolling and Assigned(Editor) then
  begin
